...
<html class="w-full ">
    ...
    <body class=" w-full  flex items-center justify-center overflow-auto	">
        ...
        <form class="w-full max-w-lg" id="form-upload-order" method="post" action="/create-order">
            ...
            <div class="flex flex-wrap -mx-3 mb-6">
                <div class="w-full px-3">
                    <button id="CatalogoGet" type="submit" class="appearance-none block w-full bg-gray-200 text-gray-600 border border-gray-600 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-900">
                        Catalogo
                    </button>
                </div>
            </div>
        ...
        <script>
            ...
            function RequestToGetCatalog(event) {
                event.preventDefault();
                axios.get("CatalogServlet").then(function (response) {
                    console.log("exito")
            ...
            var botonGetCatalog = document.getElementById("CatalogoGet");
            botonGetCatalog.addEventListener("click", RequestToGetCatalog)
